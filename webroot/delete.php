...
class CommentDelete extends ReactComponent {
    ...
    public function render() {
        if ($this->state_opened) {
            ?>
            <div onsubmit="<?= $this->onSubmit([$this, 'handleSubmit'], 'submitForm') ?>">
                削除しても良いですか？<br/>
                <input type="button" value="削除する" onclick="<?= $this->onSubmitLink('doDelete') ?>" />
                <input type="button" value="キャンセル" onclick="<?= $this->onSubmitLink('doCancel') ?>" />
            </div>
            <?
        } else {
            ?>
            <a href="javascript:<?= $this->onSubmitLink('doOpen') ?>">x</a>
            <?
        }
    }
}
